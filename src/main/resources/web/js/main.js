...
var AppRouter = Backbone.Router.extend({
    ...
    test:function () {
        ...
        $('#content').html(new TestView({model:t1}).render().el);
        $('ul.nav > li').removeClass('active');
        $('li > a[href*="/test"]').parent().addClass('active');
    },
    /*
    testDetails:function (id) {
        //console.log("switching to single host view");
        var t = tl.get(id);
        $('#content').html(new TestView({model:t}).render().el);
        $('ul.nav > li').removeClass('active');
        $('li > a[href*="/tests"]').parent().addClass('active');
    },*/
});
...
tpl.loadTemplates(['home', 'status', 'topology', 'header', 'switch', 'switch-list', 'switch-list-item', 'host', 'host-list', 'host-list-item', 'port-list', 'port-list-item', 'flow-list', 'flow-list-item','test'],
    function () {
        app = new AppRouter();
